...
code_lines = [
    ...
    "subprocess.run(['pip', 'install', '-q', 'fastapi', 'uvicorn', 'transformers', 'accelerate', 'torch', 'autoawq'], check=True)\n",
    ...
    "time.sleep(4)\n",
    "print('=== Starting Qwen 2.5 7B AWQ Server (Qwen/Qwen2.5-7B-Instruct-AWQ) ===')\n",
    ...
    "print('Waiting up to 240s for Qwen 2.5 7B AWQ server startup...')\n",
    "ready = False\n",
    "for i in range(240):\n",
    "    try:\n",
    "        r = requests.get('http://localhost:8000/v1/models', timeout=2)\n",
    "        if r.status_code == 200:\n",
    "            print(f'=== QWEN 2.5 7B AWQ SERVER IS UP AND READY (after {i*2}s) ===')\n",
    "            print(r.json())\n",
    "            ready = True\n",
    "            break\n",
    "    except Exception:\n",
    ...
    "    print('=== Running Qwen 2.5 7B AWQ Autonomous Cyber Match ===')\n",
    "    res = subprocess.run(\n",
...
print('Updated clean_nb.py for Qwen 2.5 7B AWQ model match.')
