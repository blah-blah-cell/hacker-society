...
class Environment:
    ...
    def setup(self, secret_flag: str, vuln_choice: int,
        ...
        # Write flag safely using in-memory tar stream via put_archive (prevents shell injection)
        import tarfile
        import io
        ...
        with tarfile.open(fileobj=tar_stream, mode='w') as tar:
            ...
            tarinfo.size = len(secret_flag.encode('utf-8'))
            tar.addfile(tarinfo, io.BytesIO(secret_flag.encode('utf-8')))

        tar_stream.seek(0)
        self.db_container.put_archive(os.path.dirname(flag_path), tar_stream)
