...
code_lines = [
    ...
    "subprocess.run(['pip', 'install', '-q', 'fastapi', 'uvicorn', 'transformers', 'accelerate', 'torch', 'bitsandbytes'], check=True)\n",
    ...
    "time.sleep(4)\n",
    "print('=== Starting Qwen 2.5 7B 8-Bit Server (Qwen/Qwen2.5-7B-Instruct) ===')\n",
    ...
    "print('Waiting up to 240s for Qwen 2.5 7B 8-bit server startup...')\n",
    "ready = False\n",
    "for i in range(240):\n",
    "    try:\n",
    "        r = requests.get('http://localhost:8000/v1/models', timeout=2)\n",
    "        if r.status_code == 200:\n",
    "            print(f'=== QWEN 2.5 7B 8-BIT SERVER IS UP AND READY (after {i*2}s) ===')\n",
    "            print(r.json())\n",
    "            ready = True\n",
    "            break\n",
    "    except Exception:\n",
    ...
    "    time.sleep(2)\n",
    "if not ready:\n",
    "    print('Server failed to start within 240s. Log:')\n",
    ...
    "    print('=== Running Qwen 2.5 7B 8-Bit Autonomous Cyber Match ===')\n",
    "    res = subprocess.run(\n",
    "        \"echo '1' | MOCK_DOCKER_NO_CONTAINERS=1 REAL_LOCAL_SHELL=1 python -m src.main --model Qwen/Qwen2.5-7B-Instruct --base-url http://localhost:8000/v1 --attackers 1 --defenders 1 --turns 3\",\n",
...
print('Updated clean_nb.py for 8-bit Qwen 2.5 7B model match.')
